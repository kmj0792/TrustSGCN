...
def extract_features(flag, train_path, fea_path, n_user):
    ...
    return df_train, features_train, mtx # train feature를 뽑음
...
def predict_FExtra_scores(flag,  fea_path,  features_train, mtx, n_user):
    ...
    if flag:
        ...
        with open(output_path, 'w') as f: # 모든 유저에 대해서 생성해야함 
            for snode in range(n_user):
                '''adj_set = set()
                for p2, sign, hop in adj_dic.get(snode):
                    adj_set.add(p2)'''
                util.save_predict(snode, mtx, fextra, f, n_user)#  test dataset의 feature추출 및 그 feature로 FExtra 부호/점수 예측 하고 파일에 저장
                pbar.update(1)
        pbar.close()
    else:
        ...
        with open(output_path, 'w') as f: # 모든 유저에 대해서 생성해야함 
            for snode in subgraph.keys():
                ...
                for p2, sign, hop in subgraph.get(snode):
                    adj_set.add(p2)
                #원래 util.save_predict(snode, mtx, fextra,predict, n_user)
                util.save_predict_save_time(snode, mtx, fextra, f, n_user, adj_set)#  test dataset의 feature추출 및 그 feature로 FExtra 부호/점수 예측 하고 파일에 저장
                pbar.update(1)
        pbar.close()
...
def process(fea_path, df_train,  n_user, sign_thres, adj_dic_all):
    ...
    with open(input_path, 'r') as f:
        ...
        for snode in adj_dic_all.keys(): #snode는 0부터 37126
            num_snode += 1.0
            ...
            #real edge엔 real sign값(+1/-1) 넣어주기
            scores_snode[real_edge] = 1 # real_sign=-1일때 임계값 못넘어서 1hop도 쪼개지는 경우 발생했었음 -> 그래서 real_sign -> 1로 수정
            signs_snode[real_edge] = real_sign
            # score 은 real edge엔 1/-1, 아니면 점수와 예측 부호
    ...
    # if RQ3_1 ==  True:
    lists_T1, lists_T2, lists_U1, lists_U2= matrix.set_Balance_mtx(all_pred_score, all_pred_sign, n_user, sign_thres, adj_dic_all)
    # lists_T1, lists_T2, lists_U1, lists_U2 = matrix.set_OBOE_mtx(all_pred_score, all_pred_sign, n_user, sign_thres, adj_dic_all)
    # lists_T1, lists_T2, lists_U1, lists_U2 = matrix.set_OBOE_mtx(all_pred_score, all_pred_sign, n_user, sign_thres, adj_dic_all)
    # lists_T1, lists_T2, lists_U1, lists_U2= matrix.set_FExtra_mtx(all_pred_score, all_pred_sign, n_user, sign_thres, adj_dic_all)
